...
public class AvailableModulesComponent extends CustomComponent {
	...
	public void loadData(MenuCommandResourceService menuCommandResourceService,
			IModuleService moduleService) throws Exception {

		List<MetadataModule> unconfiguredModules = new ArrayList<MetadataModule>();

		List<MetadataModule> availableModules = moduleService
				.getMetadataModules();
		...
		for (MetadataModule metadataModule : availableModules) {
			...
			try {
				menuCommandResourceService.getById(metadataModule.getId(),
		...
		availableModuleTableContainer = new BeanItemContainer<MetadataModule>(
				MetadataModule.class, unconfiguredModules);
		availableModuleTable
				.setContainerDataSource(availableModuleTableContainer);

		availableModuleTable
				.setVisibleColumns(new String[] { "id", "version" });
		availableModuleTable.setColumnHeaders(new String[] { "Identificador",
				"Versi—n" });
	}
