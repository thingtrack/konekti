...
public class JobView extends AbstractView 
	...
	private void initView() {
		...
		jobToolbar = new JobToolbar(3, bsJob, viewContainer, scheduler);
	...
	public void startJobButtonClick(JobToolbar.ClickNavigationEvent event) {
		Job editingJob = (Job) event.getRegister();		
		editingJob.setActive(true);
		
		try {
			jobService.save(editingJob);
			refreshDataGridView(editingJob);
		} catch (Exception e) {
			throw new RuntimeException("¡No se pudo arrancar el job!", e);
		}
	...
	public void stopJobButtonClick(JobToolbar.ClickNavigationEvent event) {
		Job editingJob = (Job) event.getRegister();		
		editingJob.setActive(false);
		
		try {
			jobService.save(editingJob);
			refreshDataGridView(editingJob);
		} catch (Exception e) {
			throw new RuntimeException("¡No se pudo parar el job!", e);
		}
	}
