...
public class AlarmToolbar extends AbstractToolbar {
	 ...
	 * visual editor.
	 */	
	public AlarmToolbar(int position, final BindingSource<?> bindingSource, IViewContainer viewContainer, final Scheduler scheduler) {
		...
		btnStartJob.setDescription("Iniciar Alarm");
		btnStopJob.setDescription("Parar Alarm");
		...
		btnStartJob.addListener(new ClickListener() {			
			public void buttonClick(ClickEvent event) {
				...
				AlarmJob alarmJobSelected = (AlarmJob) bindingSource.getItemId();
				
				try {
					scheduler.resumeJob(new JobKey(alarmJobSelected.getAlarmName(), alarmJobSelected.getAlarmGroup()));
					
				} catch (SchedulerException e) {
					throw new RuntimeException("¡No se pudo parar la alarma seleccionada!", e);
				}
				
				if (listenerStartJobButton != null)
					listenerStartJobButton.startJobButtonClick(new ClickNavigationEvent(event.getButton(), event.getComponent() , null, alarmJobSelected, 0));					
		...
		btnStopJob.addListener(new ClickListener() {			
			public void buttonClick(ClickEvent event) {
				...
				try {
					scheduler.pauseJob(new JobKey(alarmJobSelected.getAlarmName(), alarmJobSelected.getAlarmGroup()));					
				...
				if (listenerStopJobButton != null)
					listenerStopJobButton.stopJobButtonClick(new ClickNavigationEvent(event.getButton(), event.getComponent() , null, alarmJobSelected, 0));					
	...
	public void bindingSourceIndexChange(IndexChangeEvent event) {
		if (bindingSource != null) {
			AlarmJob alarmJobSelected = (AlarmJob)event.getRegister();
			
			if(alarmJobSelected == null)
				return;
			
			if (alarmJobSelected.getActive()) {
	...
	private HorizontalLayout buildToolbarLayout() {		
		...
		btnStartJob.setCaption("Iniciar Alarma");
		...
		btnStopJob.setCaption("Parar Alarma");
