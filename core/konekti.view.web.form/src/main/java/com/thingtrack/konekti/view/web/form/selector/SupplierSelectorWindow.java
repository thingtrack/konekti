...
public class SupplierSelectorWindow extends Window {
	...
	private Supplier domainEntity;
	
	// selector service
	private SupplierService supplierService;
	
	// selector datasource
	private BindingSource<Supplier> bsClient = new BindingSource<Supplier>(Supplier.class, 0);
	 ...
	 * visual editor.
	 */
	public SupplierSelectorWindow(final Window parentWindow, Supplier entity, final CloseWindowDialogListener listener) {
		buildMainLayout();
		...
		dgSupplier.addListener(new ItemClickListener() {			
			@Override
	...
	private void initView() {
		try {	
			...
			dgSupplier.setSelectable(true);
			dgSupplier.setFilterBarVisible();
			
			bsClient.addNestedContainerProperty("supplierType.description");
			bsClient.addNestedContainerProperty("supplierGroup.description");
			
			dgSupplier.setBindingSource(bsClient);			
			
			dgSupplier.setVisibleColumns(new String[] { "supplierGroup.description", "code", "name", "vat", "supplierType.description", "active" } );       
			dgSupplier.setColumnHeaders(new String[] { "Grupo", "Codigo", "Nombre", "CIF", "Tipo", "Activo" } );
		}
		catch(Exception e) {
			throw new RuntimeException("¡Error al crear el grid de Proveedores!", e);
		}
	}
	...
	private void refreshBindindSource() {
		try {		
			...
			bsClient.addAll(supplierService.getAll());
	...
	private void getServices() {
		try {
			BundleContext bundleContext = FrameworkUtil.getBundle(SupplierSelectorWindow.class).getBundleContext();
			
			ServiceReference supplierServiceReference = bundleContext.getServiceReference(SupplierService.class.getName());
			this.supplierService = SupplierService.class.cast(bundleContext.getService(supplierServiceReference));
	...
	public class CloseWindowDialogEvent extends CloseEvent {
		...
		public CloseWindowDialogEvent(Component source, DialogResult dialogResult, Supplier domainEntity) {
			super(source);
        ...
        * @return the Source of the event.
        */
		public Supplier getDomainEntity() {
